...
class DatabaseWrapper:
    ...
    def get_eventi_per_organizzatore(self, org): 
        return self.fetch_query("SELECT id, titolo, descrizione, data_evento, luogo, categoria, prezzo, posti_totali, posti_disponibili FROM eventi WHERE organizzatore_id = %s", (org,))
    ...
    # MODIFICA: Rimossa la clausola AND segnalata = FALSE
    def get_recensioni_evento(self, ev_id): 
        return self.fetch_query("SELECT * FROM recensioni WHERE evento_id = %s", (ev_id,))
    
    def get_recensione_by_id(self, r_id): return self.fetch_query("SELECT * FROM recensioni WHERE id = %s", (r_id,))
